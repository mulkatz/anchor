...
export const FeedbackDialog: FC<FeedbackDialogProps> = ({ onClose }) => {
  ...
  return (
    <div className="relative p-6">
      {/* Close button - positioned absolutely at top right edge */}
      <button
        onClick={async () => {
          await light();
          onClose();
        }}
        className="absolute right-2 top-2 rounded-full p-2 text-mist-white/60 transition-all duration-300 hover:bg-glass-bg-hover hover:text-mist-white"
      >
        <X size={24} />
      </button>
      ...
      <div className="mb-4 pt-2">
        <h2 className="text-center text-xl font-semibold text-mist-white">{t('feedback.title')}</h2>
      </div>
      ...
      <div className="mb-4 flex gap-2 rounded-xl bg-void-blue p-1">
        <button
          ...
          className={cn(
            ...
            kind === 'idea' ? 'bg-biolum-cyan text-void-blue shadow-glow-sm' : 'text-mist-white/60'
          )}
        >
          {t('feedback.idea')}
        </button>
        <button
          ...
          className={cn(
            'flex-1 rounded-lg px-4 py-2 text-sm font-semibold',
            'transition-all duration-300 ease-viscous',
            kind === 'bug' ? 'bg-biolum-cyan text-void-blue shadow-glow-sm' : 'text-mist-white/60'
          )}
        >
          {t('feedback.bug')}
        </button>
      ...
      <textarea
        ...
        rows={5}
        maxLength={1000}
        placeholder={kind === 'idea' ? t('feedback.ideaPlaceholder') : t('feedback.bugPlaceholder')}
        className={cn(
